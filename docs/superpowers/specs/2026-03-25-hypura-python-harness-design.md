...
├── harness_daemon.py               ← FastAPI中央デーモン (port 18790)
...
harness_daemon.py（FastAPI, port 18790, asyncio）
...
FastAPI アプリ。uvicorn で起動。port 18790。
...
**Codex呼び出し方式（フォールバック時）:**

```python
# 方式A: Claude Code CLI
subprocess.run(["claude", "-p", task, "--output-format", "json"], ...)

# 方式B: Codex CLI
subprocess.run(["codex", "--quiet", task], ...)

# 方式C: openai-codex API (OpenClaw /agent エンドポイント経由)
httpx.post("http://127.0.0.1:<openclaw_port>/agent", json={"task": task, "model": "gpt-5.4"})
```
...
`vendor/ShinkaEvolve` を `sys.path` 経由でインポート。`local_openai` プロバイダーを `OPENAI_API_BASE=http://localhost:11434/v1` で Ollama に向ける。
