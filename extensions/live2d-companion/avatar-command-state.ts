import type { AvatarCommand, CompanionAvatarStateUpdate } from "./bridge/event-types.js";
import type { CompanionAssetType } from "./companion-asset-manifest.js";

function expressionForAsset(
  expression: string,
  assetType: CompanionAssetType | null | undefined,
): string {
...
export function buildOptimisticAvatarCommandState(params: {
  ...
  assetType?: CompanionAssetType | null;
  currentEmotion?: string | null;
  now?: number;
}): CompanionAvatarStateUpdate | null {
  const { command, assetType } = params;
  const now = params.now ?? Date.now();
  let update: CompanionAvatarStateUpdate | null = null;
