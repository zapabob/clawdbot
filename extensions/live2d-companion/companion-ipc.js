...
import { isCompanionIpcRequestEnvelope, } from "./companion-ipc-protocol.js";
...
function createCompanionPipeId(stateDir) {
    return createHash("sha1")
        .update(path.resolve(stateDir))
        .digest("hex")
        .slice(0, 16);
}
export function resolveCompanionIpcPath(stateDir) {
...
export function resolveCompanionIpcAuthFilePath(stateDir) {
    return path.join(stateDir, AUTH_FILE_NAME);
}
async function readExistingAuthMetadata(stateDir) {
    try {
...
export async function ensureCompanionIpcAuthMetadata(stateDir) {
    ...
    await fs.writeFile(resolveCompanionIpcAuthFilePath(stateDir), JSON.stringify(metadata, null, 2), "utf-8");
    return metadata;
}
async function removePosixSocketIfPresent(pipePath) {
    if (process.platform === "win32") {
        return;
    }
    try {
...
export async function sendCompanionIpcRequest(params) {
