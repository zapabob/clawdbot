 ...
 */
export function startFlagWatcher(
  stateDir: string,
  webContentsRef: () => Electron.WebContents | null,
): void {
  ...
  let emotionDebounce: ReturnType<typeof setTimeout> | null = null;

  async function readAndSend<T>(filePath: string, ipcChannel: string): Promise<void> {
    try {
  ...
  ipcMain.on(IPC_CHANNELS.STT_RESULT, (_event, transcript: string) => {
