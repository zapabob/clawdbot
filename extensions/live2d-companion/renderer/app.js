...
const PERMISSION_CAPABILITIES = ["mic", "camera", "screen", "tab-follow"];
class CameraHandler {
  videoEl;
  canvasEl;
  stream = null;
...
async function main() {
  const companionConfig = await window.companionBridge.getConfig();
  const configuredAvatarType = companionConfig.avatarType ?? "auto";
  const importMode =
  ...
  async function syncDiscoveredModel(modelPath) {
