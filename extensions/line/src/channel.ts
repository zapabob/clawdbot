...
/**
 * Ebbinghaus forgetting curve stability constant (minutes).
 * R(t) = exp(-t / STABILITY) — retention after t minutes of silence.
 * At 30 min the context weight halves; at 100 min it's ~3 %.
 */
const EBBINGHAUS_STABILITY_MINUTES = 43.67; // ln(2) * 63 ≈ natural half-life of ~30 min

async function writeCompanionLineEvent(text: string, senderId: string): Promise<void> {
  const stateDir = process.env.OPENCLAW_STATE_DIR ?? ".openclaw-desktop";
  const flagPath = path.join(stateDir, "companion_line_event.json");

  // Read previous event to compute decayed context weight
  let contextDecay = 0;
  try {
    const prev = JSON.parse(await fs.readFile(flagPath, "utf-8")) as {
      timestamp?: number;
      memory_weight?: number;
    };
    if (prev.timestamp && prev.memory_weight != null) {
      const elapsedMinutes = (Date.now() - prev.timestamp) / 60_000;
      contextDecay = prev.memory_weight * Math.exp(-elapsedMinutes / EBBINGHAUS_STABILITY_MINUTES);
    }
  } catch {
    // No previous event or unreadable — start fresh
  }
  ...
  const data = {
    ...
    senderId,
    timestamp: Date.now(),
    /** Current message has full weight 1.0; context from prior messages decays per Ebbinghaus */
    memory_weight: 1.0,
    context_decay: Math.round(contextDecay * 1000) / 1000,
  };
  await fs.writeFile(flagPath, JSON.stringify(data), "utf-8").catch(() => {});
}
...
export const linePlugin: ChannelPlugin<ResolvedLineAccount> = {
  ...
  gateway: {
    startAccount: async (ctx) => {
      ...
      // Companion integration: write LINE events to flag file for Live2D companion
      // We wrap the monitor's incoming message hook by patching the runtime message handler
      const originalOnMessage = (ctx.runtime as Record<string, unknown>).onMessage as
        | ((event: { source?: { userId?: string }; message?: { text?: string } }) => unknown)
        | undefined;
      if (typeof originalOnMessage === "function") {
        (ctx.runtime as Record<string, unknown>).onMessage = (event: {
          ...
          if (text) {
            void writeCompanionLineEvent(text, senderId);
          }
