...
import { __testing as bundledRuntimeDepsInstallTesting } from "./bundled-runtime-deps-install.js";
import {
...
beforeEach(() => {
  bundledRuntimeDepsInstallTesting.setProcessLaunchersForTest({
    spawn: spawnMock,
...
afterEach(() => {
  bundledRuntimeDepsInstallTesting.resetProcessLaunchersForTest();
  vi.restoreAllMocks();
  spawnMock.mockReset();
...
describe("installBundledRuntimeDeps", () => {
  ...
  it("isolates pnpm installs from an enclosing workspace", () => {
    ...
    fs.writeFileSync(
      path.join(pnpmBinDir, process.platform === "win32" ? "pnpm.exe" : "pnpm"),
      "#!/bin/sh\n",
      "utf8",
    );
