...
import { calculateRetrievability, shouldKeepMemory, type EpisodicMemory } from "./retention.js";
...
export class MemoryIndexManager {
  ...
  // --- Episodic Memory (Ebbinghaus) ---
  public addEpisodicMemory(content: string, importance: number = 0.5): string {
    const id = randomUUID();
    const now = Date.now();
    this.db.prepare(`
      INSERT INTO episodic_memories (id, content, importance, repetition_count, last_recalled_at, created_at)
      VALUES (?, ?, ?, ?, ?, ?)
    `).run(id, content, importance, 1, now, now);
    return id;
  }

  public getEpisodicMemories(threshold: number = 0.1): EpisodicMemory[] {
    const rows = this.db.prepare(`SELECT * FROM episodic_memories`).all() as EpisodicMemory[];
    const retained = rows.filter(m => shouldKeepMemory(m, threshold));
    
    // Update recall for accessed memories? 
    // Typically only update if specifically "retrieved" for a purpose, not just listing.
    return retained;
  }

  public pruneEpisodicMemories(threshold: number = 0.1): number {
    const rows = this.db.prepare(`SELECT * FROM episodic_memories`).all() as EpisodicMemory[];
    let deleted = 0;
    const deleteParams: string[] = [];
    
    for (const m of rows) {
      if (!shouldKeepMemory(m, threshold)) {
        deleteParams.push(m.id);
        deleted++;
      }
    }
    ...
    if (deleteParams.length > 0) {
      ...
      this.db.prepare(`DELETE FROM episodic_memories WHERE id IN (${placeholders})`).run(...deleteParams);
    }
