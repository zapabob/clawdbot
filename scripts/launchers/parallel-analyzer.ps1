...
if (-not (Get-Module -ListAvailable PSScriptAnalyzer)) {
    Write-Host "[PS1] PSScriptAnalyzer is not installed on this host. Skipping PowerShell analysis." -ForegroundColor Yellow
    exit 0
}
...
while ($PendingFiles.Count -gt 0 -or $Jobs.Count -gt 0) {
    ...
    if ($Jobs.Count -gt 0) {
        ...
        $Finished = Wait-Job -Job $Jobs -Any -Timeout $WaitTimeout
            ...
            foreach ($j in $Finished) {
                ...
                if ($Output) {
                    foreach ($result in @($Output)) {
                        ...
                        Write-Host ("`n[PS1_FAIL] File: {0}" -f $result.Path) -ForegroundColor Yellow
