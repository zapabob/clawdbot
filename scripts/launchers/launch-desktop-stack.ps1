param(
    ...
    # Run `pnpm build` once before spawning Gateway/TUI so `dist/` is fresh (reduces first-connect delay when dist is stale).
    [switch]$PreBuild
)
...
foreach ($key in $mergedEnv.Keys) {
    if ($null -ne $mergedEnv[$key]) {
        $processEnv[$key] = [string]$mergedEnv[$key]
    }
}

# Dynamic overlays (computed at launch; always win over file)
$processEnv["OPENCLAW_GATEWAY_PORT"]   = [string]$GatewayPort
$processEnv["CLAWDBOT_GATEWAY_PORT"]   = [string]$GatewayPort
$processEnv["OPENCLAW_GATEWAY_BIND"]   = $GatewayBind
$processEnv["CLAWDBOT_GATEWAY_BIND"]   = $GatewayBind
$processEnv["OPENCLAW_GATEWAY_MODE"]   = $GatewayMode
$processEnv["CLAWDBOT_GATEWAY_MODE"]   = $GatewayMode
$processEnv["OPENCLAW_GATEWAY_TOKEN"]  = $token
$processEnv["OPENCLAW_PROFILE"]        = $StackProfile
$processEnv["OPENCLAW_STATE_DIR"]      = $desktopStateDir
$processEnv["OPENCLAW_CONFIG_PATH"]    = $desktopConfigPath
$processEnv["OPENCLAW_LOCAL_URL"]      = $localGatewayUrl
$processEnv["OPENCLAW_BROWSER_URL"]    = $localGatewayUrl
$processEnv["OPENCLAW_GATEWAY_WS_URL"] = $wsGatewayUrl
$processEnv["OPENCLAW_DESKTOP_LAUNCHER"] = "scripts/launchers/launch-desktop-stack.ps1"
...
Write-Host ("  PreBuild: {0}" -f ($(if ($PreBuild) { "yes (pnpm build before stack)" } else { "no" })))
Write-Host ""
...
if (-not $SkipHypura) {
    ...
    if (Test-HypuraRunning) {
        ...
        if ($hypuraCmd) {
            Write-Host "  [Hypura  ] Not running. Starting hypura serve (env injected)..." -ForegroundColor DarkCyan
            Start-StackProcess -Title "Hypura provider" `
                -WorkingDirectory $ProjectDir `
                -WindowStyle "Minimized" `
                -NoExit:$false `
                -EnvironmentOverrides $processEnv `
                -CommandParts @($hypuraCmd.Source, "serve")
...
# --- Optional: sync dist before Gateway/TUI (avoids embedded "dist is stale" build on cold start) ---
if ($PreBuild) {
    Assert-DependenciesReady -ProjectDir $ProjectDir
    Write-Host ""
    Write-Step "  [build   ] PreBuild: pnpm build..." "Yellow"
    Push-Location $ProjectDir
    try {
        $buildArgs = @($launcher.Prefix + @("build"))
        & $launcher.FilePath @buildArgs
    ...
    Write-Host "  [build   ] PreBuild complete." -ForegroundColor Green
    Write-Host ""
}
...
if (-not $SkipGateway) {
    ...
    if (-not $SkipNgrok) {
        Write-Host "  [Ngrok   ] Waiting for Gateway TCP listen on $GatewayPort..." -ForegroundColor DarkCyan
        $gwDeadline = [DateTime]::Now.AddSeconds(90)
        $gwListen = $false
        ...
        if (-not $gwListen) {
            Write-Host "  [Ngrok   ] WARNING: Gateway not listening on $GatewayPort within 90s; starting ngrok anyway (may show ERR_NGROK_8012 until gateway binds)." -ForegroundColor Yellow
        } else {
