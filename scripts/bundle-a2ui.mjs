...
const a2uiRendererTsconfig = path.join(a2uiRendererDir, "tsconfig.json");
...
async function main() {
  ...
  const hasRendererTsconfig = await pathExists(a2uiRendererTsconfig);
  const hasAppDir = await pathExists(a2uiAppDir);
  const hasOutputFile = await pathExists(outputFile);
  if (!hasRendererDir || !hasRendererTsconfig || !hasAppDir) {
    if (hasOutputFile) {
