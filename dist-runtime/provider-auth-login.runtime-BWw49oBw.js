...
import {
  gh as updateConfig,
  wm as ensureGlobalUndiciEnvProxyDispatcher,
...
import {
  _ as exchangeChutesCodeForTokens,
  g as CHUTES_AUTHORIZE_ENDPOINT,
  v as generateChutesPkce,
  y as parseOAuthCallbackInput,
...
import { a as upsertAuthProfile, s as ensureAuthProfileStore } from "./profiles-DothReVm.js";
...
} from "./provider-openai-codex-oauth-tls-DHMQmpYk.js";
  ...
  try {
    ...
    const creds = await loginOpenAICodex({
      ...
      onManualCodeInput: isRemote
        ? async () => await onPrompt({ message: manualInputPromptMessage })
        : void 0,
...
async function githubCopilotLoginCommand(opts, runtime) {
  ...
  if (
    ensureAuthProfileStore(void 0, { allowKeychainPrompt: false }).profiles[profileId] &&
    !opts.yes
  )
    note(
