...
function resolveCatalogChannelEntry(raw, cfg) {
  ...
  return listChannelPluginCatalogEntries({
    workspaceDir: cfg ? resolveWorkspaceDir(cfg) : void 0,
  }).find((entry) => {
...
async function resolveInstallableChannelPlugin(params) {
  ...
  const catalogEntry =
    ...
    (params.channelId ? getChannelPluginCatalogEntry(params.channelId, { workspaceDir }) : void 0);
  const channelId =
    params.channelId ??
