...
import {
  Ir as loadSessionEntry,
  Lr as migrateAndPruneGatewaySessionStoreKey,
  Tu as requestHeartbeatNow,
  Vm as resolveOutboundTarget,
  en as agentCommandFromIngress,
  gc as deliverOutboundPayloads,
  hc as buildOutboundSessionContext,
...
import { dn as loadOrCreateDeviceIdentity } from "./method-scopes-Le0rX1x3.js";
...
import {
  d as parseMessageWithAttachments,
  f as formatForLog,
  i as registerApnsRegistration,
  u as normalizeRpcAttachmentsToChatAttachments,
} from "./push-apns-B2mY7wQ8.js";
...
import { c as normalizeMainKey, f as scopedHeartbeatWakeOptions } from "./session-key-0JD9qg4o.js";
...
const handleNodeEvent = async (ctx, nodeId, evt) => {
  switch (evt.event) {
    ...
    case "agent.request": {
      ...
      let channel =
        normalizeChannelId(typeof link?.channel === "string" ? link.channel.trim() : "") ?? void 0;
      ...
      const receiptText =
        (typeof link?.receiptText === "string" ? link.receiptText.trim() : "") ||
    ...
    case "notifications.changed": {
      ...
      const { canonicalKey: sessionKey } = loadSessionEntry(
        normalizeNonEmptyString(obj.sessionKey) ?? `node-${nodeId}`,
      );
