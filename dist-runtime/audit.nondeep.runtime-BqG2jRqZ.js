...
import {
  a as collectSandboxBrowserHashLabelFindings,
  c as readConfigSnapshotForAudit,
  i as collectPluginsTrustFindings,
  o as collectStateDeepFilesystemFindings,
  s as collectWorkspaceSkillSymlinkEscapeFindings,
...
import { n as isDangerousNetworkMode, r as normalizeNetworkMode } from "./network-mode-JwypQ_rG.js";
import {
  r as resolveNodeCommandAllowlist,
  t as DEFAULT_DANGEROUS_NODE_COMMANDS,
} from "./node-command-policy-DZBBCvLb.js";
import "./includes-scan-Cy5yfDHR.js";
import {
  r as pickSandboxToolPolicy,
  t as isToolAllowedByPolicies,
} from "./tool-policy-match-53jrVIH7.js";
...
function listPotentialMultiUserSignals(cfg) {
  ...
  const inspectSection = (section, basePath) => {
    ...
    if (
      (Array.isArray(section.allowFrom) ? section.allowFrom : []).some(
        (entry) => String(entry).trim() === "*",
      )
    )
      out.add(`${basePath}.allowFrom includes "*"`);
    if (
      (Array.isArray(section.groupAllowFrom) ? section.groupAllowFrom : []).some(
        (entry) => String(entry).trim() === "*",
      )
    )
    ...
    if (dm && typeof dm === "object") {
      ...
      if (
        (Array.isArray(dmSection.allowFrom) ? dmSection.allowFrom : []).some(
          (entry) => String(entry).trim() === "*",
        )
      )
...
function collectSandboxDangerousConfigFindings(cfg) {
  ...
  if (browserExposurePaths.length > 0)
    findings.push({
      ...
      remediation:
        "Set sandbox.browser.network to a dedicated bridge network (recommended default: openclaw-sandbox-browser), or set sandbox.browser.cdpSourceRange (for example 172.21.0.1/32) to restrict container-edge CDP ingress.",
    });
  return findings;
}
