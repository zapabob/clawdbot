...
async function loadPendingDeliveries(stateDir) {
  ...
  for (const file of files)
    if (file.endsWith(".delivered")) await unlinkBestEffort(path.join(queueDir, file));
  const entries = [];
  for (const file of files) {
...
async function recoverPendingDeliveries(opts) {
  ...
  for (let i = 0; i < pending.length; i++) {
    ...
    if (entry.retryCount >= 5) {
      opts.log.warn(
        `Delivery ${entry.id} exceeded max retries (${entry.retryCount}/5) — moving to failed/`,
      );
...
export {
  ackDelivery as a,
  failDelivery as c,
  recoverPendingDeliveries as i,
  loadPendingDeliveries as l,
  isEntryEligibleForRecoveryRetry as n,
  enqueueDelivery as o,
  isPermanentDeliveryError as r,
  ensureQueueDir as s,
