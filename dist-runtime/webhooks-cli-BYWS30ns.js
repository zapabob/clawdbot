...
import {
  C as parseTopicPath,
  S as normalizeServePath,
  _ as buildGogWatchStartArgs,
  a as ensureTopic,
  b as mergeHookPresets,
  ...
  v as buildTopicPath,
  w as resolveGmailHookRuntimeConfig,
  x as normalizeHooksPath,
  y as generateHookToken,
} from "./gmail-setup-utils-Z0TEwq8s.js";
import {
...
async function startGmailWatch(cfg, fatal = false) {
  const result = await runCommandWithTimeout(["gog", ...buildGogWatchStartArgs(cfg)], {
    timeoutMs: 12e4,
  });
  if (result.code !== 0) {
