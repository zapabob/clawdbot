import {
  jr as classifySessionKey,
  zr as resolveSessionModelRef,
} from "./account-resolution-YAil9v6G.js";
import { n as formatTimeAgo } from "./format-relative-CiLbs-fS.js";
import { n as info } from "./globals-BKVgh_pY.js";
...
import { r as resolveSessionStoreTargets } from "./sessions-CD_-8zJN.js";
...
function resolveSessionDisplayModel(cfg, row, defaults) {
  return (
    resolveSessionModelRef(cfg, row, parseAgentSessionKey(row.key)?.agentId).model ?? defaults.model
  );
}
...
async function sessionsCommand(opts, runtime) {
  ...
  const configContextTokens =
    cfg.agents?.defaults?.contextTokens ?? lookupContextTokens(displayDefaults.model) ?? 2e5;
...
export {
  ...
  toSessionDisplayRows as c,
  formatSessionKeyCell as i,
  resolveSessionStoreTargetsOrExit as l,
  formatSessionAgeCell as n,
  resolveSessionDisplayDefaults as o,
