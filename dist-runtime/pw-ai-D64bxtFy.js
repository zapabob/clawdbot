...
import {
  D as withNoProxyForCdpUrl,
  E as markPwAiLoaded,
  F as resolveStrictExistingPathsWithinRoot,
  G as withBrowserNavigationPolicy,
  H as assertBrowserNavigationAllowed,
  I as matchBrowserUrlPattern,
  M as DEFAULT_TRACE_DIR,
  N as DEFAULT_UPLOAD_DIR,
  S as getChromeWebSocketUrl,
  U as assertBrowserNavigationRedirectChainAllowed,
  V as BrowserTabNotFoundError,
  W as assertBrowserNavigationResultAllowed,
  a as parseRoleRef,
  c as appendCdpPath,
  d as normalizeCdpHttpBaseForJsonEndpoints,
  i as getRoleSnapshotStats,
  l as fetchJson,
  n as buildRoleSnapshotFromAiSnapshot,
  o as formatAriaSnapshot,
  p as withCdpSocket,
  r as buildRoleSnapshotFromAriaSnapshot,
  s as normalizeCdpWsUrl,
  u as getHeadersWithAuth,
  ...
  const connectWithRetry = async () => {
    ...
    for (let attempt = 0; attempt < 3; attempt += 1)
      try {
        ...
        const endpoint =
          (await getChromeWebSocketUrl(normalized, timeout).catch(() => null)) ?? normalized;
...
async function findPageByTargetIdViaTargetList(pages, targetId, cdpUrl) {
  return matchPageByTargetList(
    pages,
    await fetchJson(appendCdpPath(normalizeCdpHttpBaseForJsonEndpoints(cdpUrl), "/json/list"), 2e3),
    targetId,
  );
}
