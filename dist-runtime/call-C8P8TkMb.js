...
import {
  ...
  l as resolveLeastPrivilegeOperatorScopesForMethod,
  ln as normalizeFingerprint,
  r as CLI_DEFAULT_OPERATOR_SCOPES,
  u as GatewayClient,
} from "./method-scopes-Le0rX1x3.js";
import { i as isSecureWebSocketUrl } from "./net-B1gQyBKw.js";
import {
...
function buildGatewayConnectionDetails(options = {}) {
  ...
  const cliUrlOverride =
    typeof options.url === "string" && options.url.trim().length > 0 ? options.url.trim() : void 0;
  const envUrlOverride = cliUrlOverride
    ? void 0
    : trimToUndefined(process.env.OPENCLAW_GATEWAY_URL);
  const urlOverride = cliUrlOverride ?? envUrlOverride;
  const remoteUrl =
    typeof remote?.url === "string" && remote.url.trim().length > 0 ? remote.url.trim() : void 0;
...
function resolveGatewayCallTimeout(timeoutValue) {
  const timeoutMs =
    typeof timeoutValue === "number" && Number.isFinite(timeoutValue) ? timeoutValue : 1e4;
  return {
    timeoutMs,
...
async function resolveGatewayCredentialsWithSecretInputs(params) {
  ...
  const remoteFromConfig =
    params.config.gateway?.mode === "remote" ? params.config.gateway?.remote : void 0;
...
async function callGatewayCli(opts) {
  return await callGatewayWithScopes(
    opts,
    Array.isArray(opts.scopes) ? opts.scopes : CLI_DEFAULT_OPERATOR_SCOPES,
  );
}
...
//#endregion
export {
  callGatewayLeastPrivilege as a,
  randomIdempotencyKey as c,
  loadGatewayTlsRuntime as d,
  callGatewayCli as i,
  resolveExplicitGatewayAuth as l,
  buildGatewayConnectionDetails as n,
  callGatewayScoped as o,
  callGateway as r,
  ensureExplicitGatewayAuth as s,
  __testing as t,
  resolveGatewayCredentialsWithSecretInputs as u,
};
