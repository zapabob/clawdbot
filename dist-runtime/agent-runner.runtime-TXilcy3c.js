...
import {
  ...
  Bu as resolveBootstrapWarningSignaturesSeen,
  Cc as applyReplyThreading,
  Cv as isTransientHttpError,
  Ec as resolveReplyToMode,
  Gi as routeReply,
  Gu as resolveFallbackTransition,
  Hv as resolveModelAuthMode,
  Jp as hasOutboundReplyContent,
  Ju as formatTokenCount,
  Ki as runEmbeddedPiAgent,
  Mc as parseReplyDirectives,
  Mu as resolvePathFromInput,
  Ns as resolveEffectiveBlockStreamingConfig,
  Qi as queueEmbeddedPiMessage,
  Qm as loadCronStore,
  Qp as resolveSendableOutboundReplyParts,
  Rc as runWithModelFallback,
  Sc as applyReplyTagsToPayload,
  Sv as isLikelyContextOverflowError,
  Tc as createReplyToModeFilterForChannel,
  Tv as isBillingErrorMessage,
  Ui as scheduleFollowupDrain,
  Uu as buildFallbackClearedNotice,
  Vi as enqueueFollowupRun,
  Wi as isRoutableChannel,
  Wu as buildFallbackNotice,
  Xu as resolveModelCostConfig,
  Yu as formatUsd,
  aa as registerAgentRunContext,
  an as getCliSessionId,
  bc as filterMessagingToolMediaDuplicates,
  bv as isCompactionFailureError,
  co as resolveOriginAccountId,
  da as estimateMessagesTokens,
  do as isReasoningTagProvider,
  dp as hasNonzeroUsage,
  fa as readPostCompactionContext,
  fp as normalizeUsage,
  io as createTypingSignaler,
  lo as resolveOriginMessageProvider,
  lp as derivePromptTokens,
  na as emitAgentEvent,
  on as setCliSessionId,
  qu as estimateUsageCost,
  sn as runCliAgent,
  so as refreshQueuedFollowupSession,
  uo as resolveOriginMessageTo,
  up as deriveSessionTotalTokens,
  wc as isRenderablePayload,
  wv as sanitizeUserFacingText,
  xb as resolveCronStyleNow,
  xc as shouldSuppressMessagingToolReplies,
  xv as isContextOverflowError,
  yc as filterMessagingToolDuplicates,
  yv as BILLING_ERROR_USER_MESSAGE,
} from "./account-resolution-YAil9v6G.js";
import "./paths-Chd_ukvM.js";
import { m as resolveRunModelFallbacksOverride } from "./agent-scope-BIySJgkJ.js";
...
} from "./block-reply-pipeline-Cjve2FZL.js";
import "./ansi-D3lUajt1.js";
import { h as isDiagnosticsEnabled, m as emitDiagnosticEvent } from "./diagnostic-Bah53Phe.js";
import "./file-identity-DgWfjfnD.js";
import "./boundary-file-read-DZTg2Wyt.js";
import "./logger-BsvC8P6f.js";
import "./exec-CbOKTdtq.js";
import { E as resolveSandboxConfigForAgent } from "./docker-Cqnwb08y.js";
...
import {
  a as SILENT_REPLY_TOKEN,
  o as isSilentReplyPrefixText,
  r as stripHeartbeatToken,
  s as isSilentReplyText,
} from "./heartbeat-Dh_uq3ba.js";
import "./env-substitution-X9lTyhgh.js";
import "./network-mode-JwypQ_rG.js";
import "./ip-CWtG939A.js";
import "./config-Cfud9qZm.js";
import { R as parseNonNegativeByteSize, s as loadConfig } from "./io-BeL7sW7Y.js";
...
import { l as lookupContextTokens } from "./level-overrides-DfXHgPB9.js";
import {
  a as isInternalMessageChannel,
  d as resolveMessageChannel,
  o as isMarkdownCapableMessageChannel,
...
import { a as isAudioFileName } from "./mime-lb_Ykmqj.js";
import {
...
import {
  a as resolveSessionTranscriptPath,
...
import { o as normalizeAnyChannelId, s as normalizeChannelId } from "./registry-B5KsIQB2.js";
import {
  a as ensureSandboxWorkspaceForSession,
  b as resolveSandboxRuntimeStatus,
} from "./sandbox-CqRzIi_i.js";
import "./session-write-lock-D4oaWfci.js";
import { n as generateSecureUuid } from "./secure-random-BvhfCHPg.js";
...
import { d as assertMediaNotDataUrl, h as resolveSandboxedMediaSource } from "./skills-DtPBimGK.js";
...
const resolveEnforceFinalTag = (run, provider) =>
    ...
    const taggedPayload = applyReplyTagsToPayload(
      {
        ...
        replyToId:
          payload.replyToId ??
          (payload.replyToCurrent === false ? void 0 : params.currentMessageId),
      },
...
function shouldRunMemoryFlush(params) {
  ...
  const totalTokens =
    (typeof override === "number" && Number.isFinite(override) && override > 0
      ? Math.floor(override)
      : void 0) ?? resolveFreshSessionTotalTokens(params.entry);
...
function loadReplyPayloadsDedupeRuntime() {
  replyPayloadsDedupeRuntimePromise ??= import("./reply-payloads-dedupe.runtime-HIjxPDYa.js");
...
function hasUnbackedReminderCommitment(text) {
  ...
  if (
    normalized.includes(
      "Note: I did not schedule a reminder in this turn, so this will not trigger automatically.".toLowerCase(),
    )
  )
    return false;
    ...
    try {
      await updateSessionStoreEntry({
        ...
        update: async (entry) => {
          ...
          else if (entry.estimatedCostUsd !== void 0)
...
function createFollowupRunner(params) {
  ...
  const sendFollowupPayloads = async (payloads, queued) => {
    ...
    for (const payload of payloads) {
      ...
      if (
        isSilentReplyText(payload.text, "NO_REPLY") &&
  ...
  return async (queued) => {
    try {
      ...
      let bootstrapPromptWarningSignaturesSeen = resolveBootstrapWarningSignaturesSeen(
        ((sessionKey ? sessionStore?.[sessionKey] : void 0) ?? sessionEntry)?.systemPromptReport,
      );
      try {
        const fallbackResult = await runWithModelFallback({
          cfg: queued.run.config,
...
async function runReplyAgent(params) {
  ...
  try {
    ...
    if (autoCompactionCount > 0) {
      ...
      if (sessionKey)
        readPostCompactionContext(process.cwd(), cfg)
          .then((contextContent) => {
