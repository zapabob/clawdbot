...
import { t as parseBooleanValue } from "./boolean-CsNbQKvJ.js";
import { t as formatCliCommand } from "./command-format-CI2Z3AdK.js";
import {
  a as allocateColor,
  c as isValidProfileName,
  d as DEFAULT_AI_SNAPSHOT_MAX_CHARS,
  i as allocateCdpPort,
  l as deriveDefaultBrowserCdpPortRange,
  ...
  s as getUsedPorts,
  t as parseHttpUrl,
  u as DEFAULT_AI_SNAPSHOT_EFFICIENT_MAX_CHARS,
} from "./config-DKL8TOiP.js";
import {
  c as hasGatewayPasswordEnvCandidate,
  d as readGatewayTokenEnv,
  l as hasGatewayTokenEnvCandidate,
} from "./credentials-ISiLam_U.js";
import { n as extractErrorCode, r as formatErrorMessage } from "./errors-CHvVoeNX.js";
import { n as runExec } from "./exec-CbOKTdtq.js";
import { c as isPathInside, s as isNotFoundPathError } from "./file-identity-DgWfjfnD.js";
import { a as openFileWithinRoot, t as SafeOpenError } from "./fs-safe-D6gPP2TP.js";
import { T as resolvePreferredOpenClawTmpDir } from "./globals-BKVgh_pY.js";
import {
  i as getImageMetadata,
  n as buildImageResizeSideGrid,
  s as resizeToJpeg,
...
import { n as extensionForMime, t as detectMime } from "./mime-lb_Ykmqj.js";
import { r as isLoopbackHost } from "./net-B1gQyBKw.js";
import { r as ensurePortAvailable } from "./ports-Xu1Y4c5L.js";
...
import {
  o as isPrivateNetworkAllowedByPolicy,
    ...
    ready: (async () => {
      try {
        ...
        throw new BrowserProfileUnavailableError(
          `Chrome MCP existing-session attach failed for profile "${profileName}". Make sure ${userDataDir ? `the configured Chromium user data dir (${userDataDir})` : "Google Chrome's default profile"} is running locally with remote debugging enabled. Details: ${String(err)}`,
        );
      }
    })(),
  };
...
async function findPageById(profileName, pageId, userDataDir) {
  const page = (await listChromeMcpPages(profileName, userDataDir)).find(
...
async function openChromeMcpTab(profileName, url, userDataDir) {
  const pages = extractStructuredPages(
    await callTool(profileName, userDataDir, "new_page", { url }),
  );
...
async function navigateChromeMcpPage(params) {
  ...
  return {
    url:
      (await findPageById(params.profileName, parsePageId(params.targetId), params.userDataDir))
        .url ?? params.url,
  };
}
async function takeChromeMcpSnapshot(params) {
  return extractSnapshot(
    await callTool(params.profileName, params.userDataDir, "take_snapshot", {
      pageId: parsePageId(params.targetId),
    }),
  );
}
...
async function evaluateChromeMcpScript(params) {
  return extractJsonMessage(
    await callTool(params.profileName, params.userDataDir, "evaluate_script", {
      pageId: parsePageId(params.targetId),
      function: params.fn,
...
async function loadPwAiModule(mode) {
  try {
    return await import("./pw-ai-D64bxtFy.js");
  } catch (err) {
...
async function writeSavedMediaBuffer(params) {
  ...
  await retryAfterRecreatingDir(params.dir, () =>
    fs$1.writeFile(dest, params.buffer, { mode: MEDIA_FILE_MODE }),
  );
  return dest;
}
...
function resolveBrowserExecutableForPlatform(resolved, platform) {
  if (resolved.executablePath) {
    if (!exists$1(resolved.executablePath))
...
async function fetchChromeVersion(cdpUrl, timeoutMs = 500, ssrfPolicy) {
  ...
  try {
    await assertCdpEndpointAllowed(cdpUrl, ssrfPolicy);
    const data = await (
      await fetchCdpChecked(appendCdpPath(cdpUrl, "/json/version"), timeoutMs, {
        signal: ctrl.signal,
      })
    ).json();
...
function createProfileAvailability({ opts, profile, state, getProfileState, setProfileRunning }) {
  ...
  const closePlaywrightBrowserConnectionForProfile = async (cdpUrl) => {
    try {
      await (
        await import("./pw-ai-D64bxtFy.js")
  ...
  const waitForChromeMcpReadyAfterAttach = async () => {
    ...
    throw new BrowserProfileUnavailableError(
      `Chrome MCP existing-session attach for profile "${profile.name}" timed out waiting for tabs to become available. Approve the browser attach prompt, keep the browser open, and retry.${detail}`,
    );
  };
  const ensureBrowserAvailable = async () => {
    ...
    if (!profileState.running)
      throw new BrowserProfileUnavailableError(
        `Port ${profile.cdpPort} is in use for profile "${profile.name}" but not by openclaw. Run action=reset-profile profile=${profile.name} to kill the process.`,
      );
...
function createProfileSelectionOps({
  ...
  const focusTab = async (targetId) => {
    ...
    if (capabilities.usesPersistentPlaywright) {
      const focusPageByTargetIdViaPlaywright = (await getPwAiModule$1({ mode: "strict" }))
  ...
  const closeTab = async (targetId) => {
    ...
    if (capabilities.usesPersistentPlaywright) {
      const closePageByTargetIdViaPlaywright = (await getPwAiModule$1({ mode: "strict" }))
...
function createProfileTabOps({ profile, state, getProfileState }) {
  ...
  const listTabs = async () => {
    ...
    if (capabilities.usesPersistentPlaywright) {
      const listPagesViaPlaywright = (await getPwAiModule$1({ mode: "strict" }))
        ?.listPagesViaPlaywright;
      if (typeof listPagesViaPlaywright === "function")
        return (await listPagesViaPlaywright({ cdpUrl: profile.cdpUrl })).map((p) => ({
          targetId: p.targetId,
          title: p.title,
          url: p.url,
          type: p.type,
    ...
    return (await fetchJson(appendCdpPath(cdpHttpBase, "/json/list")))
      .map((t) => ({
        targetId: t.id ?? "",
        title: t.title ?? "",
        url: t.url ?? "",
  ...
  const enforceManagedTabLimit = async (keepTargetId) => {
    ...
    for (const tab of candidates.slice(0, excessCount))
      fetchOk(appendCdpPath(cdpHttpBase, `/json/close/${tab.targetId}`)).catch(() => {});
  };
  ...
  const openTab = async (url) => {
    ...
    if (capabilities.usesPersistentPlaywright) {
      const createPageViaPlaywright = (await getPwAiModule$1({ mode: "strict" }))
    ...
    const created = await fetchJson(endpoint, CDP_JSON_NEW_TIMEOUT_MS, { method: "PUT" }).catch(
      async (err) => {
...
async function stopBrowserRuntime(params) {
  ...
  try {
    await (await import("./pw-ai-D64bxtFy.js")).closePlaywrightBrowserConnection();
  } catch {}
}
//#endregion
...
function getHeadersWithAuth(url, headers = {}) {
  ...
  try {
    const parsed = new URL(url);
    if (Object.keys(mergedHeaders).some((key) => key.toLowerCase() === "authorization"))
...
function resolveSnapshotPlan(params) {
  ...
  const maxCharsRaw =
    typeof params.query.maxChars === "string" ? Number(params.query.maxChars) : void 0;
...
//#endregion
export {
  resolveConfiguredSecretInputString as $,
  getMediaDir as A,
  normalizeBrowserFormFieldValue as B,
  parseBrowserMajorVersion as C,
  withNoProxyForCdpUrl as D,
  markPwAiLoaded as E,
  resolveStrictExistingPathsWithinRoot as F,
  withBrowserNavigationPolicy as G,
  assertBrowserNavigationAllowed as H,
  matchBrowserUrlPattern as I,
  ensureBrowserControlAuth as J,
  deleteBridgeAuthForPort as K,
  getBrowserProfileCapabilities as L,
  DEFAULT_TRACE_DIR as M,
  DEFAULT_UPLOAD_DIR as N,
  MEDIA_MAX_BYTES as O,
  resolveExistingPathsWithinRoot as P,
  mergeGatewayTailscaleConfig as Q,
  DEFAULT_FILL_FIELD_TYPE as R,
  getChromeWebSocketUrl as S,
  resolveGoogleChromeExecutableForPlatform as T,
  assertBrowserNavigationRedirectChainAllowed as U,
  BrowserTabNotFoundError as V,
  assertBrowserNavigationResultAllowed as W,
  ensureGatewayStartupAuth as X,
  resolveBrowserControlAuth as Y,
  mergeGatewayAuthConfig as Z,
  startBrowserControlServiceFromConfig as _,
  parseRoleRef as a,
  createBrowserRouteContext as b,
  appendCdpPath as c,
  normalizeCdpHttpBaseForJsonEndpoints as d,
  resolveConfiguredSecretInputWithFallback as et,
  redactCdpUrl as f,
  createBrowserControlContext as g,
  createBrowserRouteDispatcher as h,
  getRoleSnapshotStats as i,
  saveMediaBuffer as j,
  cleanOldMedia as k,
  fetchJson as l,
  fetchBrowserJson as m,
  buildRoleSnapshotFromAiSnapshot as n,
  formatAriaSnapshot as o,
  withCdpSocket as p,
  setBridgeAuthForPort as q,
  buildRoleSnapshotFromAriaSnapshot as r,
  normalizeCdpWsUrl as s,
  registerBrowserRoutes as t,
  resolveRequiredConfiguredSecretRefInputString as tt,
  getHeadersWithAuth as u,
  createBrowserRuntimeState as v,
  readBrowserVersion as w,
  movePathToTrash as x,
  stopBrowserRuntime as y,
  normalizeBrowserFormField as z,
};
