...
import {
  D_ as moveSingleAccountChannelSectionToDefaultAccount,
  Dr as formatUsageReportLines,
  Er as loadProviderUsageSummary,
  Ic as resolveCommandSecretRefsViaGateway,
  cu as resolveMessageActionDiscoveryForPlugin,
  jc as resolveMessageChannelSelection,
  su as createMessageActionDiscoveryContext,
...
import {
  i as describeBinding,
  n as applyAgentBindings,
  t as requireValidConfigSnapshot,
} from "./config-validation-BizX2kDI.js";
...
import {
  a as isCatalogChannelInstalled,
...
import {
  f as getChannelPlugin,
  m as normalizeChannelId,
  p as listChannelPlugins,
...
import {
  n as buildReadOnlySourceChannelAccountSnapshot,
  t as buildChannelAccountSnapshot,
} from "./status-C9liWluS.js";
...
function resolveCatalogChannelEntry(raw, cfg) {
  ...
  return listChannelPluginCatalogEntries({
    workspaceDir: cfg ? resolveAgentWorkspaceDir(cfg, resolveDefaultAgentId(cfg)) : void 0,
  }).find((entry) => {
...
async function channelsAddCommand(opts, runtime = defaultRuntime, params) {
  ...
  if (shouldUseWizard(params)) {
    const [{ buildAgentSummaries }, { setupChannels }] = await Promise.all([
      import("./agents.config-ChunHb6V.js"),
      import("./onboard-channels-CK983RbG.js"),
    ]);
  ...
  const loadScopedPlugin = async (channelId, pluginId) => {
    ...
    const { loadChannelSetupPluginRegistrySnapshotForChannel } =
      await import("./plugin-install-Jhe-8xqJ.js");
...
async function resolveChannelReports(params) {
  ...
  for (const accountId of accountIds) {
    ...
    const actions = Array.from(
      new Set(["send", "broadcast", ...discoveredActions.map((action) => String(action))]),
    );
    reports.push({
      plugin,
      channel: plugin.id,
      accountId,
      accountName:
        typeof resolvedAccount.name === "string" ? resolvedAccount.name?.trim() || void 0 : void 0,
      configured,
      enabled,
