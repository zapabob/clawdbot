...
import {
  n as formatPermissionRemediation,
  r as inspectPathPermissions,
  t as formatPermissionDetail,
} from "./audit-fs-BVqUNCSg.js";
...
import {
  ...
  r as isInterpreterLikeAllowlistPattern,
...
function collectBrowserControlFindings(cfg, env) {
  ...
  for (const name of Object.keys(resolved.profiles)) {
    ...
    if (isPrivateNetworkAllowedByPolicy(resolved.ssrfPolicy) && isBlockedHostnameOrIp(url.hostname))
      findings.push({
...
async function maybeProbeGateway(params) {
  ...
  const authResolution =
    !isRemoteMode || (isRemoteMode && !remoteUrlRaw)
...
async function runSecurityAudit(opts) {
  ...
  if (
    ...
    (context.plugins !== void 0 || hasPotentialConfiguredChannels(cfg, env))
  ) {
