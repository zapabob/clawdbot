...
import {
  a as resolveCoreToolProfilePolicy,
  t as CORE_TOOL_GROUPS,
} from "./tool-catalog-BACJpgR-.js";
import { l as normalizeSecretInputString } from "./types.secrets-BEA4gMCN.js";
import { y as resolveUserPath } from "./utils-DGUUVa38.js";
import {
  n as resolveWindowsSpawnProgram,
  t as materializeWindowsSpawnProgram,
} from "./windows-spawn-DmN2W3yK.js";
...
async function readRegistry() {
  return {
    entries: (await readRegistryFromFile(SANDBOX_REGISTRY_PATH, "fallback")).entries.map((entry) =>
...
function formatBindBlockedError(params) {
  if (params.reason.kind === "non_absolute")
    return /* @__PURE__ */ new Error(
      `Sandbox security: bind mount "${params.bind}" uses a non-absolute source path "${params.reason.sourcePath}". Only absolute POSIX paths are supported for sandbox binds.`,
    );
  ...
  return /* @__PURE__ */ new Error(
    `Sandbox security: bind mount "${params.bind}" ${verb} blocked path "${params.reason.blockedPath}". Mounting system directories (or Docker socket paths) into sandbox containers is not allowed. Use project-specific paths instead (e.g. /home/user/myproject).`,
  );
}
/**
...
async function ensureSandboxContainer(params) {
  ...
  let registryEntry;
  if (hasContainer) {
    registryEntry = (await readRegistry()).entries.find(
...
//#endregion
export {
  compileGlobPatterns as A,
  expandPolicyWithPluginGroups as B,
  updateRegistry as C,
  resolveSandboxScope as D,
  resolveSandboxConfigForAgent as E,
  SANDBOX_AGENT_WORKSPACE_MOUNT as F,
  resolveToolProfilePolicy as G,
  stripPluginOnlyAllowlist as H,
  SANDBOX_BROWSER_SECURITY_HASH_EPOCH as I,
  applyOwnerOnlyToolPolicy as L,
  DEFAULT_SANDBOX_BROWSER_IMAGE as M,
  DEFAULT_SANDBOX_COMMON_IMAGE as N,
  isToolAllowed as O,
  DEFAULT_SANDBOX_IMAGE as P,
  buildPluginToolGroups as R,
  updateBrowserRegistry as S,
  resolveSandboxBrowserDockerCreateConfig as T,
  expandToolGroups as U,
  mergeAlsoAllowPolicy as V,
  normalizeToolName as W,
  slugifySessionKey as _,
  execDockerRaw as a,
  removeBrowserRegistryEntry as b,
  readDockerPort as c,
  validateNetworkMode as d,
  resolveSandboxHostPathViaExistingAncestor as f,
  resolveSandboxWorkspaceDir as g,
  resolveSandboxScopeKey as h,
  execDocker as i,
  matchesAnyGlobPattern as j,
  resolveSandboxToolPolicyForAgent as k,
  appendWorkspaceMountArgs as l,
  resolveSandboxAgentId as m,
  dockerContainerState as n,
  readDockerContainerEnvVar as o,
  splitSandboxBindSpec as p,
  ensureSandboxContainer as r,
  readDockerContainerLabel as s,
  buildSandboxCreateArgs as t,
  getBlockedBindReason as u,
  readBrowserRegistry as v,
  computeSandboxBrowserConfigHash as w,
  removeRegistryEntry as x,
  readRegistry as y,
  collectExplicitAllowlist as z,
};
