...
} from "./env-substitution-X9lTyhgh.js";
...
import {
  a as ConfigIncludeError,
  c as resolveConfigIncludes,
  s as readConfigIncludeFileWithGuards,
  t as getBlockedNetworkModeReason,
} from "./network-mode-JwypQ_rG.js";
import {
...
import {
  a as shouldDeferShellEnvFallback,
  c as isDangerousHostEnvOverrideVarName,
  i as resolveShellEnvFallbackTimeoutMs,
  l as isDangerousHostEnvVarName,
  o as shouldEnableShellEnvFallback,
  r as loadShellEnvFallback,
  u as normalizeEnvVarKey,
} from "./shell-env-BOjFl6MZ.js";
import { a as hasConfiguredSecretInput, i as coerceSecretRef } from "./types.secrets-BEA4gMCN.js";
import {
  D as isPlainObject$2,
  d as isRecord$2,
  g as resolveConfigDir,
  y as resolveUserPath,
} from "./utils-DGUUVa38.js";
...
function normalizeTalkSection(value) {
  ...
  const hasNormalizedShape =
    typeof source.provider === "string" || isPlainObject$1(source.providers);
  const normalized = {};
...
function applyContextPruningDefaults(cfg) {
  ...
  if (authMode === "api_key") {
    ...
    const isAnthropicCacheRetentionTarget = (parsed) =>
      Boolean(
        parsed &&
...
const resolveDefaultAgentIdFromRaw = (raw) => {
  ...
  const firstEntry = list.find(
    (entry) => isRecord$2(entry) && typeof entry.id === "string" && entry.id.trim() !== "",
  );
...
const ensureAgentEntry = (list, id) => {
  ...
  const existing = list.find(
    (entry) => isRecord$2(entry) && typeof entry.id === "string" && entry.id.trim() === normalized,
  );
...
const LEGACY_CONFIG_MIGRATIONS_PART_2 = [
  {
    ...
    apply: (raw, changes) => {
      ...
      const legacyImageModel =
        typeof agent.imageModel === "string" ? String(agent.imageModel) : void 0;
      ...
      if (routingAgents) {
        for (const [rawId, entryRaw] of Object.entries(routingAgents)) {
          ...
          if (legacySandbox) {
            ...
            if (sandboxTools) {
              mergeMissing(
                ensureRecord$1(ensureRecord$1(ensureRecord$1(target, "tools"), "sandbox"), "tools"),
                sandboxTools,
              );
...
function hasLegacyThreadBindingTtlInAccounts(value) {
  ...
  return Object.values(value).some((entry) =>
    hasLegacyThreadBindingTtl(isRecord$1(entry) ? entry.threadBindings : void 0),
  );
}
  ...
  for (let i = 0; i < command.length; i += 1) {
    ...
    if (inHeredocBody) {
      if (ch === "\n" || ch === "\r") {
        ...
        if (current) {
          if (
            (current.stripTabs ? heredocLine.replace(/^\t+/, "") : heredocLine) ===
            current.delimiter
          )
...
function normalizeLegacyWebSearchConfigRecord(raw) {
  ...
  for (const providerId of ["firecrawl", "gemini", "grok", "kimi", "perplexity"]) {
...
const ModelProviderSchema = z
  ...
  .strict();
const BedrockDiscoverySchema = z
  .object({
...
function resolveIMessageAttachmentRoots(params) {
  return mergeInboundPathRoots(
    resolveIMessageAccountConfig(params)?.attachmentRoots,
...
const BlueBubblesGroupConfigSchema = z
  ...
  .strict();
const BlueBubblesAccountSchemaBase = z
  .object({
    name: z.string().optional(),
    capabilities: z.array(z.string()).optional(),
    markdown: MarkdownConfigSchema,
...
const McpConfigSchema = z
  .object({ servers: z.record(z.string(), McpServerSchema).optional() })
  .strict()
  .optional();
  ...
  .object({
    ...
    plugins: z
      .object({
        ...
        installs: z
          .record(z.string(), z.object({ ...PluginInstallRecordShape }).strict())
          .optional(),
      })
      .strict()
      .optional(),
...
function validateConfigObjectWithPluginsBase(raw, opts) {
  ...
  const hasExplicitPluginsConfig =
    isRecord$2(raw) && Object.prototype.hasOwnProperty.call(raw, "plugins");
  ...
  const ensureRegistry = () => {
    ...
    const registry = loadPluginManifestRegistry({
      ...
      workspaceDir:
        resolveAgentWorkspaceDir(effectiveConfig, resolveDefaultAgentId(effectiveConfig)) ?? void 0,
      env: opts.env,
    });
  ...
  if (config.channels && isRecord$2(config.channels))
...
function createConfigIO(overrides = {}) {
  ...
  const configPath =
    ...
    ).find((candidate) => deps.fs.existsSync(candidate)) ?? requestedConfigPath;
  function loadConfig() {
    try {
      ...
      const readResolution = resolveConfigForRead(
        resolveConfigIncludesForRead(deps.json5.parse(raw), configPath, deps),
        deps.env,
      );
      ...
      if (ownerDisplaySecretResolution.generatedSecret) {
        ...
        if (!AUTO_OWNER_DISPLAY_SECRET_PERSIST_IN_FLIGHT.has(configPath)) {
          ...
          writeConfigFile(cfgWithOwnerDisplaySecret, { expectedConfigPath: configPath })
            .then(() => {
...
async function writeConfigFile(cfg, options = {}) {
  ...
  const sameConfigPath =
    options.expectedConfigPath === void 0 || options.expectedConfigPath === io.configPath;
...
//#endregion
export {
  applyConfigOverrides as $,
  resolveDiscordPreviewStreamMode as $t,
  resolveTelegramCustomCommands as A,
  POSIX_SHELL_WRAPPERS as At,
  BlockStreamingCoalesceSchema as B,
  unwrapKnownDispatchWrapperInvocation as Bt,
  DiscordConfigSchema as C,
  resolveAvatarMime as Cn,
  resolveCommandResolutionFromArgv as Ct,
  TelegramConfigSchema as D,
  resolvePolicyTargetResolution as Dt,
  SlackConfigSchema as E,
  resolvePolicyTargetCandidatePath as Et,
  resolveIMessageRemoteAttachmentRoots as F,
  unwrapKnownShellMultiplexerInvocation as Ft,
  SecretProviderSchema as G,
  resolveSafeBinProfiles as Gt,
  GroupPolicySchema as H,
  DEFAULT_SAFE_BINS as Ht,
  normalizeScpRemoteHost as I,
  POSIX_INLINE_COMMAND_FLAGS as It,
  parseByteSize as J,
  ensureControlUiAllowedOriginsForNonLoopbackBind as Jt,
  requireOpenAllowFrom as K,
  applyMergePatch as Kt,
  normalizeScpRemotePath as L,
  POWERSHELL_INLINE_COMMAND_FLAGS as Lt,
  isInboundPathAllowed as M,
  extractShellWrapperInlineCommand as Mt,
  mergeInboundPathRoots as N,
  hasEnvManipulationBeforeShellWrapper as Nt,
  TELEGRAM_COMMAND_NAME_PATTERN as O,
  resolveExecutableFromPathEnv as Ot,
  resolveIMessageAttachmentRoots as P,
  isShellWrapperExecutable as Pt,
  listBundledWebSearchPluginIds as Q,
  mapStreamingModeToSlackLegacyDraftStreamMode as Qt,
  parseNonNegativeByteSize as R,
  resolveInlineCommandMatch as Rt,
  OpenClawSchema as S,
  looksLikeAvatarPath as Sn,
  resolveApprovalAuditCandidatePath as St,
  SignalConfigSchema as T,
  resolveNormalizedAccountEntry as Tn,
  resolveExecutionTargetResolution as Tt,
  MarkdownConfigSchema as U,
  SAFE_BIN_PROFILES as Ut,
  DmPolicySchema as V,
  normalizeExecutableToken as Vt,
  ReplyRuntimeConfigSchemaShape as W,
  normalizeSafeBinProfileFixtures as Wt,
  resolvePluginWebSearchConfig as X,
  formatSlackStreamModeMigrationMessage as Xt,
  migrateLegacyWebSearchConfig as Y,
  isSafeExecutableValue as Yt,
  validateJsonSchemaValue as Z,
  formatSlackStreamingBooleanMigrationMessage as Zt,
  normalizeOpenClawVersionBase as _,
  isAvatarHttpUrl as _n,
  buildEnforcedShellCommand as _t,
  getRuntimeConfigSnapshot as a,
  resolveNormalizedProviderModelMaxTokens as an,
  parseConfigPath as at,
  validateConfigObjectRawWithPlugins as b,
  isSupportedLocalAvatarExtension as bn,
  splitCommandChain as bt,
  parseConfigJson5 as c,
  normalizeTalkSection as cn,
  getTrustedSafeBinDirs as ct,
  readConfigFileSnapshot as d,
  resolveSubagentMaxConcurrent as dn,
  normalizeTrustedSafeBinDirs as dt,
  resolveSlackNativeStreaming as en,
  getConfigOverrides as et,
  readConfigFileSnapshotForWrite as f,
  resolveOwnerDisplaySetting as fn,
  validateSafeBinArgv as ft,
  writeConfigFile as g,
  isAvatarDataUrl as gn,
  analyzeShellCommand as gt,
  setRuntimeConfigSnapshotRefreshHandler as h,
  AVATAR_MAX_BYTES as hn,
  analyzeArgvCommand as ht,
  createConfigIO as i,
  createConfigRuntimeEnv as in,
  getConfigValueAtPath as it,
  DEFAULT_IMESSAGE_ATTACHMENT_ROOTS as j,
  extractShellWrapperCommand as jt,
  normalizeTelegramCommandName as k,
  resolveExecWrapperTrustPlan as kt,
  projectConfigOntoRuntimeSourceSnapshot as l,
  resolveActiveTalkProviderConfig as ln,
  isTrustedSafeBinPath as lt,
  setRuntimeConfigSnapshot as m,
  withBundledPluginEnablementCompat as mn,
  normalizeSafeBinName as mt,
  clearConfigCache as n,
  resolveTelegramPreviewStreamMode as nn,
  setConfigOverride as nt,
  getRuntimeConfigSourceSnapshot as o,
  DEFAULT_TALK_PROVIDER as on,
  setConfigValueAtPath as ot,
  resolveConfigSnapshotHash as p,
  withBundledPluginAllowlistCompat as pn,
  listRiskyConfiguredSafeBins as pt,
  MODEL_APIS as q,
  applyLegacyMigrations as qt,
  clearRuntimeConfigSnapshot as r,
  collectDurableServiceEnvVars as rn,
  unsetConfigOverride as rt,
  loadConfig as s,
  buildTalkConfigResponse as sn,
  unsetConfigValueAtPath as st,
  ConfigRuntimeRefreshError as t,
  resolveSlackStreamingMode as tn,
  resetConfigOverrides as tt,
  readBestEffortConfig as u,
  resolveAgentMaxConcurrent as un,
  listWritableExplicitTrustedSafeBinDirs as ut,
  validateConfigObject as v,
  isAvatarImageDataUrl as vn,
  isWindowsPlatform as vt,
  IMessageConfigSchema as w,
  resolveAccountEntry as wn,
  resolveExecutionTargetCandidatePath as wt,
  validateConfigObjectWithPlugins as x,
  isWorkspaceRelativeAvatarPath as xn,
  matchAllowlist as xt,
  validateConfigObjectRaw as y,
  isPathWithinRoot as yn,
  resolvePlannedSegmentArgv as yt,
  ToolPolicySchema as z,
  unwrapDispatchWrappersForResolution as zt,
};
