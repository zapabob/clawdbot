...
var Parser = class {
  ...
  *[fixedLengthFrameSync](ignoreNextFrame) {
    ...
    this._codecParser[logWarning](
      `Missing ${frame$1} at ${frameLength} bytes from current position.`,
      `Dropping current ${frame$1} and trying again.`,
    );
...
var CodecFrame = class extends Frame {
  static *[getFrame](Header, Frame, codecParser, headerCache, readOffset) {
    ...
    if (headerValue) {
      ...
      return new Frame(
        headerValue,
        (yield* codecParser[readRawData](frameLengthValue, readOffset))[subarray$1](
          0,
          frameLengthValue,
        ),
        samplesValue,
...
var ID3v2 = class ID3v2 {
  static *getID3v2Header(codecParser, headerCache, readOffset) {
    ...
    header[length$1] =
      headerLength + ((data[6] << 21) | (data[7] << 14) | (data[8] << 7) | data[9]);
...
var MPEGHeader = class MPEGHeader extends CodecHeader {
  static *[getHeader](codecParser, headerCache, readOffset) {
    ...
    header[frameLength$1] = Math.floor(
      (125 * header[bitrate$1] * header[samples$1]) / header[sampleRate$1] + header[framePadding$1],
    );
...
const channelModeValues = {
  ...
  448: {
    [channels$1]: 8,
    [description$1]: getChannelMapping(
      8,
      channelMappings[1][3],
      channelMappings[2][0],
      channelMappings[3][0],
        ...
        while (nextHeaderOffset <= MAX_FLAC_FRAME_SIZE) {
          if (
            ...
            if (!this._codecParser._flushing)
              frameData = frameData[subarray$1](0, nextHeaderOffset);
...
var OggPage = class OggPage extends Frame {
  static *[getFrame](codecParser, headerCache, readOffset) {
    ...
    if (header) {
      ...
      const rawDataValue = (yield* codecParser[readRawData](totalLength, 0))[subarray$1](
        0,
        totalLength,
      );
...
var OpusHeader = class OpusHeader extends CodecHeader {
  static [getHeaderFromUint8Array](dataValue, packetData, headerCache) {
    ...
    const key =
      bytesToString(dataValue[subarray$1](0, header[length$1])) +
      bytesToString(packetData[subarray$1](0, packetLength));
    const cachedHeader = headerCache[getHeader](key);
    ...
    if (header["channelMappingFamily"] in channelMappingFamilies) {
      header[channelMode$1] =
        channelMappingFamilies[header[channelMappingFamily$1]][header[channels$1] - 1];
...
var OpusParser = class extends Parser {
  ...
  [parseOggPage](oggPage) {
    if (oggPage["pageSequenceNumber"] === 0) {
      ...
      oggPage[codecFrames$1] = frameStore.get(oggPage)[segments$1].map((segment) => {
...
var VorbisParser = class extends Parser {
  ...
  [parseOggPage](oggPage) {
    ...
    for (const oggPageSegment of frameStore.get(oggPage)[segments$1])
...
var OggStream = class {
  ...
  _checkPageSequenceNumber(oggPage) {
    if (
      ...
      this._codecParser[logWarning](
        ...
        `Expected: ${this._pageSequenceNumber + 1}, Got: ${oggPage[pageSequenceNumber$1]}`,
      );
  ...
  _parsePage(oggPage) {
    ...
    let offset = 0;
    oggPageStore[segments$1] = headerData[pageSegmentTable$1].map((segmentLength) =>
      oggPage[data$1][subarray$1](offset, (offset += segmentLength)),
    );
    if (this._continuedPacket["length"]) {
      oggPageStore[segments$1][0] = concatBuffers(
        this._continuedPacket,
        oggPageStore[segments$1][0],
      );
      this._continuedPacket = new uint8Array();
    }
    if (headerData[pageSegmentBytes][headerData[pageSegmentBytes]["length"] - 1] === 255)
      this._continuedPacket = concatBuffers(this._continuedPacket, oggPageStore[segments$1].pop());
    if (this._previousAbsoluteGranulePosition !== null)
      oggPage[samples$1] = Number(
...
var CodecParser = class {
  ...
  *[readRawData](minSize = 0, readOffset = 0) {
  ...
  [mapCodecFrameStats](frame) {
    ...
    frame[header$1][bitrate$1] =
      frame["duration"] > 0 ? Math.round(frame[data$1][length$1] / frame[duration$1]) * 8 : 0;
      ...
      if (frame["isLastPage"]) {
        ...
        frame[codecFrames$1].forEach((codecFrame) => {
          ...
          if (absoluteGranulePositionSamples < untrimmedCodecSamples) {
            codecFrame[samples$1] =
              absoluteGranulePositionSamples > 0 ? absoluteGranulePositionSamples : 0;
            codecFrame[duration$1] =
              (codecFrame[samples$1] / codecFrame[header$1][sampleRate$1]) * 1e3;
          }
