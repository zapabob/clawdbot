...
import {
  Hl as applyCloudflareAiGatewayConfig,
...
import {
  a as resolveCustomProviderId,
  n as applyCustomApiConfig,
  r as parseNonInteractiveCustomApiFlags,
  t as CustomApiError,
} from "./onboard-custom-DMtNOQ1w.js";
...
import {
  a as setCloudflareAiGatewayConfig,
  i as applyLitellmConfig,
  o as setLitellmApiKey,
  r as normalizeApiKeyTokenProviderAuthChoice,
  t as resolvePreferredProviderForAuthChoice,
...
async function applySimpleNonInteractiveApiKeyChoice(params) {
  ...
  if (
    !(await params.maybeSetResolvedApiKey(resolved, (value) =>
      setLitellmApiKey(value, void 0, params.apiKeyStorageOptions),
    ))
  )
    return null;
...
async function loadPluginProviderRuntime() {
  return import("./auth-choice.plugin-providers.runtime-BPHXiGS4.js");
}
...
async function applyNonInteractivePluginProviderChoice(params) {
  ...
  const preferredProviderId =
    (params.authChoice.startsWith(PROVIDER_PLUGIN_CHOICE_PREFIX)
  ...
  const providerChoice = resolveProviderPluginChoice({
    providers: resolvePluginProviders({
      ...
      workspaceDir,
      onlyPluginIds: preferredProviderId
...
async function applyNonInteractiveAuthChoice(params) {
  ...
  if (Object.prototype.hasOwnProperty.call(REMOVED_MINIMAX_CHOICES, authChoice)) {
    ...
    runtime.error(
      `"${authChoice}" is no longer supported. Use --auth-choice ${replacement} instead.`,
    );
    runtime.exit(1);
    return null;
  }
