...
import {
  n as FILE_SECRET_REF_ID_PATTERN,
...
Type.Union([RequestFrameSchema, ResponseFrameSchema, EventFrameSchema], { discriminator: "type" });
//#endregion
...
var GatewayClient = class {
  constructor(opts) {
    ...
    this.requestTimeoutMs =
      typeof opts.requestTimeoutMs === "number" && Number.isFinite(opts.requestTimeoutMs)
        ? Math.max(1, Math.min(Math.floor(opts.requestTimeoutMs), 2147483647))
        : 3e4;
  }
  start() {
  ...
  selectConnectAuth(role) {
    ...
    const authBootstrapToken =
      !explicitGatewayToken && !resolvedDeviceToken ? explicitBootstrapToken : void 0;
    return {
      authToken,
      authBootstrapToken,
  ...
  queueConnect() {
    ...
    const connectChallengeTimeoutMs =
      typeof rawConnectDelayMs === "number" && Number.isFinite(rawConnectDelayMs)
        ? Math.max(250, Math.min(1e4, rawConnectDelayMs))
        : 2e3;
  ...
  async request(method, params, opts) {
    ...
    const timeoutMs =
      opts?.timeoutMs === null
        ? null
        : typeof opts?.timeoutMs === "number" && Number.isFinite(opts.timeoutMs)
          ? Math.max(1, Math.min(Math.floor(opts.timeoutMs), 2147483647))
          : expectFinal
            ? null
            : this.requestTimeoutMs;
...
const METHOD_SCOPE_BY_NAME = new Map(
  Object.entries(METHOD_SCOPE_GROUPS).flatMap(([scope, methods]) =>
    methods.map((method) => [method, scope]),
  ),
);
...
//#endregion
export {
  validateExecApprovalsNodeSetParams as $,
  applyInputProvenanceToUserMessage as $t,
  validateConfigPatchParams as A,
  validateSessionsResetParams as At,
  validateCronRunsParams as B,
  validateTalkSpeakParams as Bt,
  validateChannelsStatusParams as C,
  validateSessionsCreateParams as Ct,
  validateChatSendParams as D,
  validateSessionsMessagesUnsubscribeParams as Dt,
  validateChatInjectParams as E,
  validateSessionsMessagesSubscribeParams as Et,
  validateConnectParams as F,
  validateSkillsInstallParams as Ft,
  validateDevicePairRejectParams as G,
  validateWebLoginWaitParams as Gt,
  validateCronUpdateParams as H,
  validateUpdateRunParams as Ht,
  validateCronAddParams as I,
  validateSkillsStatusParams as It,
  validateDeviceTokenRotateParams as J,
  validateWizardStartParams as Jt,
  validateDevicePairRemoveParams as K,
  validateWizardCancelParams as Kt,
  validateCronListParams as L,
  validateSkillsUpdateParams as Lt,
  validateConfigSchemaLookupResult as M,
  validateSessionsSendParams as Mt,
  validateConfigSchemaParams as N,
  validateSessionsUsageParams as Nt,
  validateConfigApplyParams as O,
  validateSessionsPatchParams as Ot,
  validateConfigSetParams as P,
  validateSkillsBinsParams as Pt,
  validateExecApprovalsNodeGetParams as Q,
  parseSessionLabel as Qt,
  validateCronRemoveParams as R,
  validateTalkConfigParams as Rt,
  validateChannelsLogoutParams as S,
  validateSessionsCompactParams as St,
  validateChatHistoryParams as T,
  validateSessionsListParams as Tt,
  validateDevicePairApproveParams as U,
  validateWakeParams as Ut,
  validateCronStatusParams as V,
  validateToolsCatalogParams as Vt,
  validateDevicePairListParams as W,
  validateWebLoginStartParams as Wt,
  validateExecApprovalResolveParams as X,
  ErrorCodes as Xt,
  validateExecApprovalRequestParams as Y,
  validateWizardStatusParams as Yt,
  validateExecApprovalsGetParams as Z,
  errorShape as Zt,
  validateAgentsFilesGetParams as _,
  validateRequestFrame as _t,
  READ_SCOPE as a,
  buildDeviceAuthPayload as an,
  validateNodeInvokeParams as at,
  validateAgentsListParams as b,
  validateSendParams as bt,
  isNodeRoleMethod as c,
  normalizeDeviceAuthScopes as cn,
  validateNodePairApproveParams as ct,
  formatValidationErrors as d,
  loadOrCreateDeviceIdentity as dn,
  validateNodePairRequestParams as dt,
  hasInterSessionUserProvenance as en,
  validateExecApprovalsSetParams as et,
  validateAgentIdentityParams as f,
  normalizeDevicePublicKeyBase64Url as fn,
  validateNodePairVerifyParams as ft,
  validateAgentsDeleteParams as g,
  validatePushTestParams as gt,
  validateAgentsCreateParams as h,
  verifyDeviceSignature as hn,
  validatePollParams as ht,
  PAIRING_SCOPE as i,
  resolveDeviceAuthConnectErrorDetailCode as in,
  validateNodeEventParams as it,
  validateConfigSchemaLookupParams as j,
  validateSessionsResolveParams as jt,
  validateConfigGetParams as k,
  validateSessionsPreviewParams as kt,
  resolveLeastPrivilegeOperatorScopesForMethod as l,
  normalizeFingerprint as ln,
  validateNodePairListParams as lt,
  validateAgentWaitParams as m,
  signDevicePayload as mn,
  validateNodeRenameParams as mt,
  APPROVALS_SCOPE as n,
  ConnectErrorDetailCodes as nn,
  validateModelsListParams as nt,
  WRITE_SCOPE as o,
  buildDeviceAuthPayloadV3 as on,
  validateNodeInvokeResultParams as ot,
  validateAgentParams as p,
  publicKeyRawBase64UrlFromPem as pn,
  validateNodePendingAckParams as pt,
  validateDeviceTokenRevokeParams as q,
  validateWizardNextParams as qt,
  CLI_DEFAULT_OPERATOR_SCOPES as r,
  resolveAuthConnectErrorDetailCode as rn,
  validateNodeDescribeParams as rt,
  authorizeOperatorScopesForMethod as s,
  normalizeDeviceAuthRole as sn,
  validateNodeListParams as st,
  ADMIN_SCOPE as t,
  normalizeInputProvenance as tn,
  validateLogsTailParams as tt,
  GatewayClient as u,
  deriveDeviceIdFromPublicKey as un,
  validateNodePairRejectParams as ut,
  validateAgentsFilesListParams as v,
  validateSecretsResolveParams as vt,
  validateChatAbortParams as w,
  validateSessionsDeleteParams as wt,
  validateAgentsUpdateParams as x,
  validateSessionsAbortParams as xt,
  validateAgentsFilesSetParams as y,
  validateSecretsResolveResult as yt,
  validateCronRunParams as z,
  validateTalkModeParams as zt,
};
