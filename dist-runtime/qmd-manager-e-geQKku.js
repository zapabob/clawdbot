...
import { u as writeFileWithinRoot } from "./fs-safe-D6gPP2TP.js";
...
import {
  a as listSessionFilesForAgent,
  b as statRegularFile,
  i as buildSessionEntry,
  r as requireNodeSqlite,
  t as extractKeywords,
  y as isFileMissingError,
} from "./query-expansion-Bjd1_3ef.js";
...
var QmdMemoryManager = class QmdMemoryManager {
  ...
  async symlinkSharedModels() {
    ...
    try {
      if (
        !(
          await fs.stat(defaultModelsDir).catch((err) => {
  ...
  isWithinWorkspace(absPath) {
    ...
    return (absPath.endsWith(path.sep) ? absPath : `${absPath}${path.sep}`).startsWith(
      normalizedWorkspace,
    );
  }
  isWithinRoot(root, candidate) {
    ...
    return (candidate.endsWith(path.sep) ? candidate : `${candidate}${path.sep}`).startsWith(
      normalizedRoot,
    );
  }
