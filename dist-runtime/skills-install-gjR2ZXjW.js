...
import {
  c as prepareArchiveDestinationDir,
  d as isWithinDir,
  l as withStagedArchiveDestination,
  n as extractArchive$1,
  s as mergeExtractedTreeIntoDestination,
  t as createTarEntryPreflightChecker,
  u as isWindowsDrivePath,
} from "./archive-Ccs4T-SG.js";
...
import {
  i as safePathSegmentHashed,
  t as assertCanonicalPathWithinBase,
...
async function installSkill(params) {
  ...
  const entry = loadWorkspaceSkillEntries(resolveUserPath(params.workspaceDir)).find(
    (item) => item.skill.name === params.skillName,
  );
  if (!entry)
    return {
      ok: false,
  ...
  if (!new Set(["openclaw-bundled", "openclaw-managed", "openclaw-extra"]).has(entry.skill.source))
    warnings.push(
