...
import {
  ...
  wd as parseTtsDirectives,
  zd as transcribeAudioFile,
...
import { a as resolveAgentDir } from "./agent-scope-BIySJgkJ.js";
...
import "./runtime-Cks9HsVs.js";
import { g as isDangerousNameMatchingEnabled } from "./dm-policy-shared-D3Y8oBe8.js";
...
var DiscordVoiceManager$1 = class {
  ...
  async join(params) {
    ...
    logVoiceVerbose(
      `join: DAVE settings encryption=${daveEncryption === false ? "off" : "on"} tolerance=${decryptionFailureTolerance ?? "default"}`,
    );
    ...
    disconnectedHandler = async () => {
      try {
        await Promise.race([
          voiceSdk.entersState(connection, voiceSdk.VoiceConnectionStatus.Signalling, 5e3),
          voiceSdk.entersState(connection, voiceSdk.VoiceConnectionStatus.Connecting, 5e3),
        ]);
      } catch {
