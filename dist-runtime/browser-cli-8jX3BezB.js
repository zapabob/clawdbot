...
import { n as info, t as danger } from "./globals-BKVgh_pY.js";
...
import {
  B as normalizeBrowserFormFieldValue,
  N as DEFAULT_UPLOAD_DIR,
  P as resolveExistingPathsWithinRoot,
  f as redactCdpUrl,
  z as normalizeBrowserFormField,
...
function registerBrowserManageCommands(browser, parentOpts) {
  ...
  tab
    ...
    .action(async (_opts, cmd) => {
      ...
      const profile = parent?.browserProfile;
      await runBrowserCommand$1(async () => {
        if (printJsonResult(parent, await callTabAction(parent, profile, { action: "new" })))
          return;
    ...
    .action(async (headersJson, opts, cmd) => {
      ...
      await runBrowserCommand(async () => {
        const headersJsonValue =
          ...
          (headersJson?.trim() ? headersJson.trim() : void 0);
        if (!headersJsonValue)
