...
import {
  ...
  Ap as stripStructuralPrefixes,
  Au as getSession,
  Av as EmbeddedBlockChunker,
  Ay as getSessionBindingService,
  B_ as getAcpSessionManager,
  Bf as resolveThreadBindingMaxAgeMsForChannel,
  Bn as rejectUnauthorizedCommand,
  Ci as stopSubagentsForRequester,
  Cr as resolveAcpSpawnRuntimePolicyError,
  Ed as resolveModelWithRegistry,
  Ei as countPendingDescendantRuns,
  F_ as resolveAcpThreadSessionDetailLines,
  Ff as formatThreadBindingDisabledError,
  G_ as requireAcpRuntimeBackend,
  Gf as resolveThreadBindingIntroText,
  Gn as handleExportSessionCommand,
  H_ as readAcpSessionEntry,
  Hi as clearSessionQueues,
  Hn as requireGatewayClientScopeForInternalChannel,
  Hu as formatTokenCount,
  I_ as isAcpEnabledByPolicy,
  If as formatThreadBindingSpawnDisabledError,
  In as handleModelsCommand,
  Is as parseActivationCommand,
  Iv as getApiKeyForModel,
  J_ as validateRuntimeCwdInput,
  Jc as scheduleGatewaySigusr1Restart,
  Jn as handleWhoamiCommand,
  Ju as formatTokenCount$1,
  K_ as parseRuntimeTimeoutSecondsInput,
  Kf as resolveThreadBindingThreadName,
  Kl as resolveFastModeState,
  Kn as handleHelpCommand,
  L_ as resolveAcpAgentPolicyError,
  Mi as listSubagentRunsForController,
  Mv as ensureOpenClawModelsJson,
  N_ as resolveAcpSessionCwd,
  Od as executePluginCommand,
  Ol as stripToolMessages,
  P_ as resolveAcpSessionIdentifierLinesFromIdentity,
  Q_ as resolveAcpSessionResolutionError,
  Qu as getLastTtsAttempt,
  R_ as resolveAcpDispatchPolicyError,
  Rf as resolveThreadBindingIdleTimeoutMsForChannel,
  Rn as buildDisabledCommandReply,
  Si as resolveSessionEntryForKey,
  Su as killProcessTree,
  Sy as createInternalHookEvent,
  Tp as setAbortMemory,
  U_ as resolveSessionStorePathForAcp,
  Uf as formatThreadBindingDurationLabel,
  Un as handleCommandsListCommand,
  Vf as resolveThreadBindingSpawnPolicy,
  Vn as requireCommandFlagEnabled,
  Vu as formatContextUsageShort,
  W_ as getAcpRuntimeBackend,
  Wn as handleContextCommand,
  X_ as validateRuntimeModelInput,
  Xi as isEmbeddedPiRunActive,
  Y_ as validateRuntimeModeInput,
  Yi as getActiveEmbeddedRunSnapshot,
  Yu as formatUsd,
  Z_ as validateRuntimePermissionProfileInput,
  Zc as triggerOpenClawRestart,
  _d as textToSpeech,
  a_ as formatConfigWriteDeniedMessage,
  bf as isTelegramExecApprovalApprover,
  bi as extractBtwQuestion,
  cd as resolveTtsConfig,
  cr as killAllControlledSubagentRuns,
  dd as setLastTtsAttempt,
  ea as waitForEmbeddedPiRunEnd,
  ed as getTtsProvider,
  fd as setSummarizationEnabled,
  fr as sendControlledSubagentMessage,
  hd as setTtsProvider,
  hr as formatRunLabel,
  i_ as canBypassConfigWritePolicy,
  ju as markExited,
  kd as matchPluginCommand,
  kp as stripMentions,
  ku as getFinishedSession,
  ld as resolveTtsPrefsPath,
  lr as killControlledSubagentRun,
  lu as createExecTool,
  ma as stripToolResultDetails,
  md as setTtsMaxLength,
  mr as formatDurationCompact,
  nd as isTtsEnabled,
  nv as toAcpRuntimeError,
  oa as compactEmbeddedPiSession,
  od as resolveTtsApiKey,
  pd as setTtsEnabled,
  pr as steerControlledSubagentRun,
  q_ as validateRuntimeConfigOptionInput,
  qi as abortEmbeddedPiRun,
  qn as handleStatusCommand,
  r_ as authorizeConfigWrite,
  rd as isTtsProviderConfigured,
  s_ as resolveConfigWriteTargetFromPath,
  sr as buildSubagentList,
  td as isSummarizationEnabled,
  tu as setPluginEnabledInConfig,
  ua as mapThinkingLevel,
  vr as sortSubagentRuns,
  wp as isAbortTrigger,
  wr as cleanupFailedAcpSpawn,
  wy as triggerInternalHook,
  xd as normalizeSpeechProviderId,
  xf as isTelegramExecApprovalClientEnabled,
  xi as formatAbortReplyText,
  xo as formatElevatedUnavailableMessage,
  yd as getSpeechProvider,
  yr as spawnSubagentDirect,
  z_ as resolveAcpDispatchPolicyMessage,
  zn as rejectNonOwnerCommand,
  zv as requireApiKey,
...
import {
  I as normalizeUsageDisplay,
  M as normalizeFastMode,
  R as resolveResponseUsageMode,
...
import {
  a as decidePreferredClawHubFallback,
  f as resolveFileNpmSpecToLocalPath,
  i as createPluginInstallLogger,
  n as buildPreferredClawHubSpec,
} from "./plugins-command-helpers-D2EceMsj.js";
import {
  i as buildNpmInstallRecordFields,
  n as persistPluginInstall,
} from "./plugins-install-persist-Bbzh3xvT.js";
...
import {
  ...
  r as loadSessionCostSummary,
} from "./session-cost-usage-C_F0S1TI.js";
import {
...
import {
  a as buildPluginStatusReport,
  i as buildPluginInspectReport,
...
import {
  $ as resolveCommandSurfaceChannel,
  A as resolveAcpCommandBindingContext,
  B as resolveDiscordChannelIdForFocus,
  C as resolveAcpHelpText,
  D as resolveAcpInstallCommandHint,
  E as withAcpCommandErrorBoundary,
  F as buildSubagentsHelp,
  G as resolveSubagentEntryForToken,
  H as resolveFocusTargetSession,
  I as formatLogLines,
  J as resolveTelegramConversationId,
  K as resolveSubagentsAction,
  L as formatTimestampWithAge,
  M as resolveMatrixConversationId,
  N as resolveMatrixParentConversationId,
  O as resolveConfiguredAcpBackendId,
  Q as resolveChannelAccountId,
  R as loadSubagentSessionEntry,
  S as resolveAcpAction,
  T as stopWithText,
  U as resolveHandledPrefix,
  V as resolveDisplayStatus,
  W as resolveRequesterSessionKey,
  X as isMatrixSurface,
  Y as isDiscordSurface,
  Z as isTelegramSurface,
  _ as parseOptionalSingleTarget,
  a as ACP_INSTALL_USAGE,
  b as parseSpawnInput,
  c as ACP_RESET_OPTIONS_USAGE,
  d as ACP_STATUS_USAGE,
  f as ACP_TIMEOUT_USAGE,
  g as formatRuntimeOptionsText,
  h as formatAcpCapabilitiesText,
  i as ACP_DOCTOR_USAGE,
  j as resolveAcpCommandConversationId,
  k as resolveAcpCommandAccountId,
  l as ACP_SESSIONS_USAGE,
  m as collectAcpErrorText,
  n as resolveBoundAcpThreadSessionKey,
  o as ACP_MODEL_USAGE,
  p as COMMAND,
  q as stopWithText$1,
  r as ACP_CWD_USAGE,
  s as ACP_PERMISSIONS_USAGE,
  t as resolveAcpTargetSessionKey,
  u as ACP_SET_MODE_USAGE,
  v as parseSetCommandInput,
  w as resolveCommandRequestId,
  x as parseSteerInput,
  y as parseSingleValueCommandInput,
  z as resolveCommandSubagentController,
} from "./targets-C1N6tW3Y.js";
...
async function runBtwSideQuestion(params) {
  ...
  if (Array.isArray(activeRunSnapshot?.messages) && activeRunSnapshot.messages.length > 0) {
    ...
    if (leafEntry?.type === "message" && leafEntry.message?.role === "user")
      if (leafEntry.parentId && sessionManager.branch) sessionManager.branch(leafEntry.parentId);
      else sessionManager.resetLeaf?.();
  }
...
const handleCompactCommand = async (params) => {
  ...
  const totalTokens =
    result.result?.tokensAfter ?? resolveFreshSessionTotalTokens(params.sessionEntry);
...
const handleUsageCommand = async (params, allowTextCommands) => {
  ...
  if (rawArgs.toLowerCase().startsWith("cost")) {
    ...
    const sessionTokens = sessionSummary?.totalTokens
      ? formatTokenCount$1(sessionSummary.totalTokens)
      : void 0;
  ...
  const current = resolveResponseUsageMode(
    params.sessionEntry?.responseUsage ??
      (params.sessionKey ? params.sessionStore?.[params.sessionKey]?.responseUsage : void 0),
  );
...
async function handleSubagentsFocusAction(ctx) {
  ...
  if (channel !== "discord" && channel !== "matrix" && channel !== "telegram")
    return stopWithText$1("⚠️ /focus is only available on Discord, Matrix, and Telegram.");
  const token = restTokens.join(" ").trim();
  if (!token)
    return stopWithText$1("Usage: /focus <subagent-label|session-key|session-id|session-label>");
  const accountId = resolveChannelAccountId(params);
  const bindingService = getSessionBindingService();
  const capabilities = bindingService.getCapabilities({
    channel,
  ...
  if (!capabilities.adapterAvailable || !capabilities.bindSupported)
    return stopWithText$1(
      `⚠️ ${channel === "discord" ? "Discord thread" : channel === "matrix" ? "Matrix thread" : "Telegram conversation"} bindings are unavailable for this account.`,
    );
  ...
  if (!bindingContext) {
    ...
    if (channel === "matrix")
      return stopWithText$1("⚠️ Could not resolve a Matrix room for /focus.");
  ...
  const boundBy =
    typeof existingBinding?.metadata?.boundBy === "string"
      ...
      : "";
  if (existingBinding && boundBy && boundBy !== "system" && senderId && senderId !== boundBy)
    return stopWithText$1(`⚠️ Only ${boundBy} can refocus this ${bindingContext.labelNoun}.`);
  ...
  if (!capabilities.placements.includes(bindingContext.placement))
    return stopWithText$1(`⚠️ ${channel} bindings are unavailable for this account.`);
  let binding;
  try {
    binding = await bindingService.bind({
  ...
  return stopWithText$1(
    `✅ ${bindingContext.placement === "child" ? `created thread ${binding.conversation.conversationId} and bound it to ${binding.targetSessionKey}` : `bound this ${bindingContext.labelNoun} to ${binding.targetSessionKey}`} (${focusTarget.targetKind}).`,
  );
}
//#endregion
...
async function handleSubagentsKillAction(ctx) {
  ...
  if (targetResolution.entry.endedAt)
    return stopWithText$1(`${formatRunLabel(targetResolution.entry)} is already finished.`);
  const controller = resolveCommandSubagentController(params, requesterKey);
...
async function handleSubagentsSendAction(ctx, steerRequested) {
  ...
  if (result.status === "timeout")
    return stopWithText$1(`⏳ Subagent still running (run ${result.runId.slice(0, 8)}).`);
  if (result.status === "error")
    return stopWithText$1(`⚠️ Subagent error: ${result.error} (run ${result.runId.slice(0, 8)}).`);
...
async function handleSubagentsUnfocusAction(ctx) {
  ...
  const channel = resolveCommandSurfaceChannel(params);
  if (channel !== "discord" && channel !== "matrix" && channel !== "telegram")
    return stopWithText$1("⚠️ /unfocus is only available on Discord, Matrix, and Telegram.");
  const accountId = resolveChannelAccountId(params);
  const bindingService = getSessionBindingService();
  ...
  if (!conversationId) {
    if (channel === "discord")
      return stopWithText$1("⚠️ /unfocus must be run inside a Discord thread.");
    if (channel === "matrix")
      return stopWithText$1("⚠️ /unfocus must be run inside a Matrix thread.");
    return stopWithText$1(
