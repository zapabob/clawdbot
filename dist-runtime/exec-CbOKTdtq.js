...
import { c as shouldLogVerbose, t as danger } from "./globals-BKVgh_pY.js";
...
function resolveNpmArgvForWindows(argv) {
  ...
  return [process$1.execPath, cliPath, ...argv.slice(1)];
}
/**
...
async function runCommandWithTimeout(argv, optionsOrTimeout) {
  ...
  const finalArgv =
    process$1.platform === "win32" ? (resolveNpmArgvForWindows(argv) ?? argv) : argv;
  const resolvedCommand = finalArgv !== argv ? (finalArgv[0] ?? "") : resolveCommand(argv[0] ?? "");
  const useCmdWrapper = isWindowsBatchCommand(resolvedCommand);
  const child = spawn(
    useCmdWrapper ? (process$1.env.ComSpec ?? "cmd.exe") : resolvedCommand,
    useCmdWrapper
...
//#endregion
export {
  spawnWithFallback as i,
  runExec as n,
  resolveWindowsCommandShim as r,
  runCommandWithTimeout as t,
};
